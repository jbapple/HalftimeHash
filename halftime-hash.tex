...
\keywords{Hashing \and Algorithms \and Randomized algorithms}
\end{abstract}
...
\]
The intuition behind this definition is that the input to be hashed might not be random, but collisions can still be made unlikely by picking randomly from a hash {\em family} independest of the input strings, rather than anchoring on a specific hash {\em function} that maps from $D$ to $C$ but does not take a seed as an input. AU hash families are useful in hash tables, where collisions slow down operations and, in extreme cases, can turn linear algorithms into quadratic ones. \cite{tabulation,rust-quadratic,impala-quadratic,algorithm-attack}
...
Other capital letters will denote sets, functions, or matrices.
Lowercase letters represent strings or non-negative integers; $n$ generally denotes the size of the input string in 64-bit words.
Arithmetic operations are over elements of $\ints_m$ or $\ints$.
...
Subscripts are used to indicate a numbered component of a sequence, starting at 0.
...
More concretely, $H(s_1,\langle{}a,b\rangle) = x$ is a tree hash execution of height 1, $H(s_1, \langle H(s_0, \langle a,b\rangle), H(s_0, \langle c,d\rangle)\rangle) = y$ is a tree hash execution of height 2, and so on.
...
NH is $2^{-32}$-AU, and is widely used in hash families such as UMAC \cite{umac}, VMAC \cite{vmac}, clhash \cite{clhash}, UMASH \cite{umash}, XXH3 \cite{xxh3}, and more, usually with $k = 64$.
...
At the leaves of the tree, HalftimeHash uses an algorithm called ``EHC'' for {\em encode-hash-combine}: \cite{ehc-nandi}

\begin{enumerate}
\item A sequence of items is encoded with an erasure code with minimum distance $d$.
\item Each item in the encoded sequence is hashed, with an independently chosen seed, to a field $F$.
  HalftimeHash uses NH for the encoding. % (the AU variant, not the A$\Delta$U one)
\item The resulting sequence of hash values is put through a linear transformation $T$ on $F^m$.
...
While {\em linear} erasure codes on $\ints_2$ exist, they do not achieve as ratio as low as $9/7$ for code with minimum distance 3. \cite{codetables.de}
...
Rahter than Carter and Wegman's breath-first approach to tree hash, which allocates linear space in the size of the input on each execution, or a recursive approach that prooved to be prohibitively slow, HalftimeHash uses an iterative depth-first post-order approach.
...
When the next call of EHC is about to be made, HalftimeHash hashes the lowest level in the stack with NH and puts it output in the next level of the stack.
...
The stack evolution is equivalent to iteration in a redundant zeroless number system in base $f$. \cite{redundant-zeroless}
...
First, the tree hash leaves data in its stack - as many as $bf$ words of data per level per tree, where $b$ is the size of a block and $f$ is \texttt{fanout}, a user-tunable parameter discussed in Section \ref{analysis}.
...
Second, it feeds every word in the remainder of the input string into {\em all three} NH executions.
...
\begin{lemma}
The tree hash portion of EHC produce is $2^{ k\lg\left\lfloor\log_f \lfloor n/b d w\rfloor\right\rfloor - 32k}$-AU.
\end{lemma}
\begin{proof}
...
Specifically, there are up to $b f$ words in each stack level, and there are $k \left\lfloor \log_f \lfloor n / b d w \rfloor \right\rfloor$ stack levels, counting each $k$ times for the $k$ different stacks.
...
This could be reduced by using the pass-through mechanism of Badger, rather than leaving data at multiple levels in the stack, as well as hashing the remaining untouched input string with another instantiation of HalftimeHash with smaller block size, but these adds complexity to the algorithm and empirically reduce performance.
...
\begin{figure}
\includegraphics[width=\textwidth]{clang-local-hh4.eps}
\caption{
...
\begin{figure*}
\begin{tabular}{cccc}
\includegraphics[width=6.0cm]{amd-16.eps}
&
\includegraphics[width=6.0cm]{amd-24.eps}
\end{tabular}
\caption{
  \label{block-size}
  AMD performance.
