...
AU hash families are useful in hash tables, where collisions slow down operations and, in extreme cases, can turn linear algorithms into quadratic ones. \cite{tabulation,rust-quadratic,impala-quadratic}
...
\item[Collision probability $\varepsilon$] is less than than $2^{-83}$ bits for input shorter than one exabyte (for the family producing 24 bytes of output).
  Shorter input strings produce lower collision probability, as do longer output strings (32 or 40 bytes).
...
HalftimeHash uses less than 1200 lines of code in C++, does not use any assembly, and makes spare use of the C++ standard library.
It can take advantage of vector ISA extensions like AVX-512, AVX2, and SSE from the x86-64 architecture, and NEON from the ARM architecture, but it can also run on the C++ virtual machine model with no architecture-specific features.
...
\captionof{figure}{
  \label{smhasher-speed}
    SomThe two fastest variants of HalftimeHash are faster than all hash families in the SMHasher suite on a CPU with AVX-512 instructions, even families that come with no AU guarantees. \protect\cite{smhasher}
...
Future potential work includes:
...
\item The size of the keys used in HalftimeHash can be lowered by using the Badger pass-through mechanism on the data left in the stack after DFS traversal.
  It could also be reduced by apply HalftimeHash recursively, with smaller block sizes, on the leftover data in the input string after the DFS traversal.
