...
Compared to the fastest universal hash functions on long strings, clhash and UMASH, HalftimeHash decreases collision probability while also increasing performance by over 70\%.
\end{abstract}
...
It also surpasses the performance of Fast Positive Hash, falkhash, xxh3, and MeowHash.
...
\begin{lemma}
  In EHC, when the matrix is over a field and the hash function used in step 2 is $\varepsilon$-AU, EHC is $\varepsilon^k$-AU.
\end{lemma}
\begin{proof}
  ...
  \begin{equation}
    \label{ehc-delta}
    \Pr_s[T|_F \bar{H}|_F(s, g(x)) = T|_F \bar{H}|_F(s, g(y))]
  \end{equation}
  where $H$ is the hash function used in step 2 of EHC.

  Since any $k$ columns of $T$ are independent, $T|_F$ is non-singular, and the equation is equivalent to $\hat{H}|_F(s, g(x)) = \hat{H}|_F(s, g(y))$, which implies
  \[
  \bigwedge_{i \in F} H(s_i, g(x)_i) = H(s_i, g(y)_i)
  \]
...
\begin{proof}
  ...
  \[
  \begin{array}{rl}
    &  \Pr_s[T|_F \bar{H}|_F(s,x) = T|_F \bar{H}|_F(s,y)]\\
    \leq & \Pr_s[{T|_F}^? T|_F \bar{H}|_F(s,x) = {T|_F}^? T|_F \bar{H}|_F(s,y)] \\
    = & \Pr_s[q2^p\bar{H}|_F(s,x) = q2^p\bar{H}|_F(s,y)] \\
    = & \Pr_s[2^p\bar{H}|_F(s,x) = 2^p\bar{H}|_F(s,y)] \\
    = & \Pr_s[\bar{H}|_F(s,x) \equiv \bar{H}|_F(s,y) \bmod 2^{64-p}] \\
...
\begin{tabular}{|r|l|}
    ...
    \hline {\bf Multiplications (each)} & $b e (w-1)$ \\
    \hline {\bf Multiplications (total)} & $b e (w-1) \lfloor n / b d w\rfloor$ \\
