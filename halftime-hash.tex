...
%% Section \ref{algo} focuses on a particular instantiation of HalftimeHash with distance $3$, producing a hash value $24$ bytes in size.
...
\begin{figure}
\includegraphics[width=\textwidth]{yes-badger.eps}
...
\begin{figure}
\includegraphics[width=\textwidth]{no-badger.eps}
...
%%     \hline
%% \end{tabular}
%%  \caption{Metrics for EHC in HalftimeHash}
%%  \label{ehc-metrics}
%% %% %\vspace{-2.5em}
%%  \end{wrapfigure}
...
%The total metrics for EHC in HalftimeHash are in Figure~\ref{ehc-metrics}.

%The total metrics for EHC in HalftimeHash are:
...
%% %% \begin{wrapfigure}[9]{L}{0.75\textwidth}
%% %%   \vspace{-2em}
%% \begin{tabular}{|r|l|}
...
%%     \hline
%% \end{tabular}
%% %% \caption{Metrics for tree hash in HalftimeHash}
%% %% \label{ehc-metrics}
%% %\end{wrapfigure}
...
In addition to the performance testing here, HalftimeHash passes all correctness tests in the SMHasher test suite; for a performance comparison, see Figure \ref{smhasher-speed} and \cite{smhasher}.
...
\includegraphics{speed-v-epsilon-amd.eps}

\includegraphics{clang-local-hh4.eps}
